...
# 이 파일의 메서드 본문만 교체하면 라우터는 영향을 받지 않는다.
#
# [설계 메모]
# - 클래스 기반(VoiceService)으로 작성. 라우터는 모듈-수준 싱글톤 인스턴스를 사용한다.
# - 추후 DB 세션 주입이 필요해지면 __init__(self, session) 으로 의존성을 받고
#   라우터에서 FastAPI Depends 로 주입하는 형태로 자연스럽게 확장된다.
# =============================================================================
...
# =============================================================================
# VoiceService — 음성/텍스트 명령 비즈니스 로직 클래스
# =============================================================================
class VoiceService:
    """음성/텍스트 명령 CRUD 책임을 가진 서비스 클래스.

    제공 메서드:
        - create_voice_command : 새 명령을 "저장한 척" 하고 더미 결과 반환.
        - list_voice_commands  : 특정 문서의 명령 이력을 더미로 반환.

    추후 실제 DB 연동 시 __init__(self, session) 으로 의존성을 받고 메서드 본문만
    실제 INSERT/SELECT 로 교체하면 된다.
    """
    ...
    def create_voice_command(
        ...
        """새 음성/텍스트 명령을 "저장한 척"하고 결과를 반환하는 Mock 메서드.
    ...
    # 메서드 2) 명령 이력 조회 (Mock)
    # -------------------------------------------------------------------------
    def list_voice_commands(self, document_id: str) -> list[dict]:
        """특정 문서에 대한 음성/텍스트 명령 이력을 조회하는 Mock 메서드.
